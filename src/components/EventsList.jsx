  ...
  return (
    <header className="flex flex-col items-center gap-4 mb-8">
      ...
      <div className="flex flex-wrap justify-center gap-4">
        <select className="bg-purple-700 text-white rounded px-4 py-2 min-w-[140px]" value={city} onChange={e => setCity(e.target.value)}>
          {CITIES.map(c => <option key={c} value={c}>{c}</option>)}
        </select>

        <select className="bg-purple-700 text-white rounded px-4 py-2 min-w-[140px]" value={freeFilter} onChange={e => setFreeFilter(e.target.value)}>
          {FREE_FILTERS.map(f => <option key={f} value={f}>{f}</option>)}
        </select>

        <select className="bg-purple-700 text-white rounded px-4 py-2 min-w-[140px]" value={genreFilter} onChange={e => setGenreFilter(e.target.value)}>
          {GENRES.map(g => <option key={g} value={g}>{g}</option>)}
        </select>

        <select className="bg-purple-700 text-white rounded px-4 py-2 min-w-[140px]" value={ageFilter} onChange={e => setAgeFilter(e.target.value)}>
